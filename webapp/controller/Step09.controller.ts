...
export default class Step09 extends Controller {
  ...
  private _setDetailArea(oContext?: Context | null): void {
    ...
    if (oDetailArea) {
      // Nếu có context → bind dữ liệu vào detailArea
      // Nếu không có → clear binding (gán null)
      oDetailArea.setBindingContext((oContext ?? null) as any);

      // Chỉ hiển thị detail area khi có context
      oDetailArea.setVisible(!!oContext);
    }

    if (oLayoutData) {
      // Nếu có context → master layout chiếm 60% màn hình và cho phép resize
      // Nếu không có context → master layout chiếm 100% và không cho resize
      oLayoutData.setSize(oContext ? "60%" : "100%");
      oLayoutData.setResizable(!!oContext);
    }

    if (oSearchField) {
      // Nếu có detail → search field rộng hơn (40%)
      // Nếu không có detail → search field hẹp lại (20%)
