...
export default class Step08 extends Controller {
  ...
  public async onDelete(): Promise<void> {
    // Lấy Table theo id "peopleList"
    const oTable = this.byId("peopleList") as Table | undefined;

    // Lấy item đang được chọn trong Table
    const oSelected = oTable?.getSelectedItem();
    if (!oSelected) return; // Nếu không có item nào được chọn → thoát

    // Lấy binding context của item được chọn
    const oContext = oSelected.getBindingContext();
    if (!oContext) return; // Nếu context không tồn tại → thoát

    // Lấy path của entity trong OData model
    const sPath = oContext.getPath();

    // Lấy UserName từ context để hiển thị trong thông báo
    const sUserName = oContext.getProperty("UserName") as string | undefined;

    // Lấy OData V2 model từ view
    const oModel = this.getView()?.getModel() as ODataModelV2 | undefined;
    if (!oModel) return; // Nếu không tìm thấy model → thoát

    // Lock UI (disable các nút Save/Refresh/Search/Sort)
    this._setBusy(true);

    try {
      // Gọi remove() của ODataModel để xóa entity
      // Bọc trong Promise để dùng async/await
      await new Promise<void>((resolve, reject) => {
        oModel.remove(sPath, {
          groupId: "$direct", // gửi ngay request, không dùng batch
          success: () => resolve(), // resolve nếu xóa thành công
          error: (oError: { message: string }) => reject(oError), // reject nếu có lỗi
        });
      });

      // Lấy message i18n thông báo xóa thành công
      const sMsg = await this._getText("deletionSuccessMessage", [sUserName]);
      // Hiển thị toast thông báo
      MessageToast.show(sMsg);

      // Cập nhật UI flag → không còn thay đổi chưa lưu
      this._setUIChanges(false);
    } catch (oError: any) {
      // Nếu xóa lỗi, đánh dấu UI có thay đổi
      this._setUIChanges(true);

      // Nếu người dùng hủy thao tác xóa
      if (oError.canceled) {
      ...
      // Nếu lỗi khác, hiển thị MessageBox với thông tin lỗi
      MessageBox.error(`${oError.message}: ${sUserName}`);
    } finally {
      // Mở lại UI (unlock các nút)
      this._setBusy(false);
    }
  }
